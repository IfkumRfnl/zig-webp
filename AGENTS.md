...
- Keep `README.MD`, `PLAN.MD`, and `PROGRESS.MD` up-to-date when behavior,
  scope, roadmap, or usage changes. `PLAN.MD` is forward-looking (scope,
  ordering, acceptance gates); completed work and dated oracle results go in
  `PROGRESS.MD`, not into `PLAN.MD`.
