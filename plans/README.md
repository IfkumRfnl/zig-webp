...
| 027 | Threading design doc in PLAN.MD (post-1.0, no code) | P3 | M | 020–023 (soft) | DONE — `threading-design-doc` branch; PLAN.MD step 14 records the opt-in caller-supplied `std.Io` + `Io.Group` model, determinism/budget/failure contracts, per-path structure, and implementation gates; no code |
